...
for n_views in [4]:
    ...
    cmd = f'CUDA_VISIBLE_DEVICES={gpu_id} python evaluation.py configs/infer.yaml n_views={n_views} infer.eval_novel_view_only=True ' \
        f'infer.ckpt_path={ckpt_path} ' \
        f'infer.dataset.dataset_name=human ' \
        f'infer.video_frames=0 infer.save_mesh=False ' \
        f'infer.save_folder=outputs/image_vis/{name}_dna_rendering_{n_views}_views infer.dataset.n_group={n_views} '
    os.system(cmd)
