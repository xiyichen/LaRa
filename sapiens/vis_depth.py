...
def img_save_and_viz(output_root, image, mask, result, output_path, seg_dir):
    if os.path.isfile(output_path):
        return
    seg_logits = F.interpolate(
        result.unsqueeze(0), size=image.shape[:2], mode="bilinear"
    ).squeeze(0)
    file_id = output_path.split('/')[-2]
    os.makedirs(os.path.join(output_root, file_id), exist_ok=True)

    depth_map = seg_logits.data.float().numpy()[0].astype(np.float16)  ## H x W
    image_name = os.path.basename(output_path)
    # print(image.shape, output_path)
    # exit()
    ...
    vis_image = np.concatenate([image, processed_depth], axis=1)
    vis_image_h = vis_image.shape[0]
    vis_image_w = vis_image.shape[1]
    vis_image = cv2.resize(vis_image, (vis_image_w//8, vis_image_h//8))
...
def main():
    ...
    exp_model = load_model('/fs/gamma-projects/3dnvs_gamma/sapiens_checkpoints/sapiens_2b_render_people_epoch_25_bfloat16.pt2', USE_TORCHSCRIPT)
    ...
    for k in d.keys():
        if 'main' in k and 'apose' not in k:
            file_ids.append(k.split('/')[-1].split('.')[0])
    print(f'PROCESSING PART {args.index}')
    ...
    for batch_idx, (file_id, batch_image_name, batch_orig_imgs, batch_imgs, batch_masks) in tqdm(
        ...
        args_list = [
            (   args.output_root,
                ...
                m,
                r,
                os.path.join(args.output_root, file_id_name, os.path.basename(img_name)),
                args.seg_dir,
            )
            for i, m, r, file_id_name, img_name in zip(
                batch_orig_imgs[:valid_images_len],
                batch_masks[:valid_images_len],
